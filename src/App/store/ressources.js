...
const ressources = createSlice({
  ...
  extraReducers:(builder) => {
    ...
    builder.addCase('current/saveImage/fulfilled', (s,a) => {
        const ind = s.memes.findIndex(x => x.id === a.payload.id);
        if(ind === -1) s.memes.push(a.payload);
        else s.memes[ind]=Object.assign(s.memes[ind], a.payload);
    })
    /*builder.addMatcher('current/*', (s,a) => {
